...
public class ServerSelectGui extends Screen {
    ...
    private void rebuild() {
        ...
        addButton(new Button(rx, ry + step, BTN_W, BTN_H,
        ...
        // Таймаут: 3 → 5 → 10 → 3
        addButton(new Button(rx, ry + step*2, BTN_W, BTN_H,
        ...
        // ── Переключатели мобов ────────────────────────────────────────────────
        // Пауки (ползунок-кнопка)
        String spiderLbl = Scanner.scanSpiders
                ? "§a🕷 Пауки: ВКЛ"
                : "§c🕷 Пауки: ВЫКЛ";
        addButton(new Button(rx, ry + step*3, BTN_W, BTN_H,
                new StringTextComponent(spiderLbl), btn -> {
            Scanner.scanSpiders = !Scanner.scanSpiders;
            rebuild();
        }));

        // Чешуйницы
        String sfLbl = Scanner.scanSilverfish
                ? "§a🐛 Чешуйницы: ВКЛ"
                : "§c🐛 Чешуйницы: ВЫКЛ";
        addButton(new Button(rx, ry + step*4, BTN_W, BTN_H,
                new StringTextComponent(sfLbl), btn -> {
            Scanner.scanSilverfish = !Scanner.scanSilverfish;
            rebuild();
        }));

        // Панды
        String pandaLbl = Scanner.scanPandas
                ? "§a🐼 Панды: ВКЛ"
                : "§c🐼 Панды: ВЫКЛ";
        addButton(new Button(rx, ry + step*5, BTN_W, BTN_H,
                new StringTextComponent(pandaLbl), btn -> {
            Scanner.scanPandas = !Scanner.scanPandas;
            rebuild();
        }));

        // ── Утилиты ───────────────────────────────────────────────────────────
        // /fly
        addButton(new Button(rx, ry + step*6, BTN_W, BTN_H,
                new StringTextComponent("§e/fly"),
                btn -> Minecraft.getInstance().player.sendChatMessage("/fly")));

        // Сброс кэша
        addButton(new Button(rx, ry + step*7, BTN_W, BTN_H,
    ...
    public void render(MatrixStack ms, int mx, int my, float pt) {
        ...
        drawCenteredString(ms, font, "§7Текущая: §b" + KrikCraftMod.currentAnarchy, width/2, 26, 0xFFFFFF);

        int lx = this.width / 2 - BTN_W - 10;
        int rx = this.width / 2 + 20;

        drawString(ms, font, "§7Серверы:",    lx, 40, 0xAAAAAA);
        drawString(ms, font, "§7Управление:", rx, 40, 0xAAAAAA);

        // Секция «Мобы»
        int mobsLabelY = 50 + (BTN_H + 4) * 3 - 10;
        drawString(ms, font, "§7─── Мобы ───", rx, mobsLabelY, 0x888888);
