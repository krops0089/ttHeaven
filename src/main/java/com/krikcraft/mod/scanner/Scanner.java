...
public class Scanner {
    ...
    private static final int RESCAN_CHUNK_TICKS = 100; // через 5 сек
    ...
    private static final int  GROUP_RADIUS        = 48;  // чуть больше для маленьких структур
    private static final long GROUP_TIMEOUT_MS    = 2000L; // быстрее отправлять
    ...
    public static void tick(Minecraft mc) {
        ...
        // Сбрасываем кэш мобов каждые 30 сек — они "забываются" и ловятся заново
    ...
    // ── PacketSniffer callback — вызывается с cx/cz чанка из пакета ────────────
    ...
    private static void scanTileEntities(Minecraft mc) {
        ...
        for (int key : new ArrayList<>(recheckChunks)) {
            ...
            // Y-цикл только для новых чанков из PacketSniffer
            for (int bx = 0; bx < 16; bx++) {
                for (int y = 0; y <= 255; y++) {
                    for (int bz = 0; bz < 16; bz++) {
                        BlockPos pos = new BlockPos(cx*16+bx, y, bz*16+bz);
                        if (spawnerCache.containsKey(pos)) continue;
                        TileEntity te = mc.world.getTileEntity(pos);
                        if (te instanceof MobSpawnerTileEntity)
                            handleSpawner(pos, "Packet");
                    }
                }
            }
        }
    ...
    private static void scanEntities(Minecraft mc) {
        ...
        AxisAlignedBB box = new AxisAlignedBB(px - ENTITY_RADIUS, 0, pz - ENTITY_RADIUS,
    ...
    private static String specialTEName(TileEntity te) {
        if (te instanceof JigsawTileEntity)        return "🧩 Dамагер";
        if (te instanceof StructureBlockTileEntity) return "🔷 Прогрузчик";
        if (te instanceof CommandBlockTileEntity)   return "⌨ Командный блок";
        return null;
    }

    private static String specialBlockName(net.minecraft.block.Block b) {
        if (b == Blocks.JIGSAW)                  return "🧩 Дамагер";
        if (b == Blocks.STRUCTURE_BLOCK)         return "🔷 Прогрузчик";
